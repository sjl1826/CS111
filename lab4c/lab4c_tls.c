...
void shut_down() {
    ...
    SSL_write(ssl_client, out_text, strlen(out_text));
    if(isLogging) {
...
int main(int argc, char* argv[]) {
    ...
    if(!SSL_set_fd(ssl_client, sockfd)) {
        fprintf(stderr, "Error: sock file descriptor not set\n");
        exit(1);
    }
    ...
    SSL_write(ssl_client, id_text, strlen(id_text));
    if(isLogging)
    ...
    while(1) {
        ...
        if ((pollVal = poll(&poll_fd, 1, 0)) < 0) {
            ...
            if(poll_fd.revents & POLLIN) {
                ...
                char command[100];
                ssize_t bytes = SSL_read(ssl_client, buffer, 256);
